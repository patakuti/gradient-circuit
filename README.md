...
- **Vibration**: the device vibrates (Web Vibration API, `web/src/haptics/vibration.ts`) when you ride a curb, drive on grass, hit the barrier, brake (not at low speed), or pull strong lateral G (3 g and up). Only the strongest applies at a time (barrier > curb > grass > lateral G > brake), and each is a different pulse pattern since the vibration strength itself can't be controlled. It stops while the settings panel is open or the app is in the background. The pulse patterns and the 3 g threshold are tuning constants, not yet confirmed on a real device.
