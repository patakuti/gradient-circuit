...
Node.js 18 以上が必要です。`npm run dev` 後、表示された URL(既定 http://localhost:5173/)をブラウザで開きます。画面左下のスライダー、またはキーボードの ↑ / W でスロットル操作ができ、車の後方から追従するカメラでコースを周回できます。1 周完了するとブラウザの開発者コンソールにログが出力されます。ブレーキ・コーナリング速度制限は初期スコープ外です(要求 4.2)。
