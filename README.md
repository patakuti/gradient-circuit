...
Use the arrow keys / WASD: `↑`/`W` for throttle, `↓`/`S` for brake, `←`/`A` to steer left, `→`/`D` to steer right. Steering ramps up smoothly while held and self-centers back to neutral when released. If you enter a corner too fast for its curvature, the car doesn't auto-brake — it pushes wide (understeer) instead, so brake early enough to hold your line. The body leans toward the outside of a turn under lateral G and lifts on whichever side rides a curb (chase view only; the cockpit view stays level), and tires squeal — higher-pitched the faster you go — as cornering approaches and exceeds the grip limit.
