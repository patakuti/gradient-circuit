...
Use the arrow keys / WASD: `↑`/`W` for throttle, `↓`/`S` for brake, `←`/`A` to steer left, `→`/`D` to steer right. Steering ramps up smoothly while held and self-centers back to neutral when released. If you enter a corner too fast for its curvature, the car doesn't auto-brake — it pushes wide (understeer) instead, so brake early enough to hold your line. The body leans toward the outside of a turn under lateral G (the chassis only; the wheels stay planted), and the whole car, wheels included, lifts on whichever side rides a curb while the other side's wheels stay on the road (chase view only; the cockpit view stays level), and the cornering-scrub sound rises in pitch with speed once you exceed the grip limit.
...
Going off-track changes behavior depending on the surface. Monaco (a street circuit) mostly has a barrier immediately outside the track, but a handful of corners have a real curb (Sainte Devote, the Fairmont/Grand Hotel hairpin, Mirabeau Bas, Portier, the first Swimming Pool esses, and Anthony Noghes) before the barrier, matching the real circuit; the Nouvelle Chicane's curb is on the right side only, and each curb zone runs a bit further through the corner exit than the corner itself, matching where the curb actually gets used. A curb corner also has a bit of extra paved margin beyond the curb before the barrier, so using the whole width of the curb doesn't put you into the barrier. Suzuka (a purpose-built circuit) has curb (0.6 m, slightly reduced grip) → grass (6 m, grip and acceleration drop sharply) → barrier, so running onto the grass noticeably slows you down while you can still steer back onto the track. Neither course lets you cross the barrier, but you can keep driving alongside it. Press `R` to reset onto the track (nearest centerline point, speed 0) — lap count and distance are preserved. The current surface type is shown in the HUD.
...
Engine, braking, and tire-scrub sounds (heard while pushing wide beyond the grip limit) play alongside curb/grass/barrier contact sounds — all procedurally generated with the Web Audio API, no external audio files. Due to browser autoplay restrictions, sound activates on your first key press. Mute it with the "mute" button at the bottom left. All sound stops while the app is in the background (switching apps, going home, screen off, or a hidden browser tab) and resumes when you come back; a muted game stays muted.
...
Procedurally generated scenery (no external 3D assets) surrounds the track, styled to match each course's character: Monaco has buildings, a tunnel section, a harbor section (water and yachts), and curb ribbons at the specific corners listed above; Suzuka has curbs (the full way around), grass, and trees. Monaco's buildings use a palette of pale tones (cream, ochre, terracotta, soft pink, etc.) evoking a Mediterranean streetscape, and Suzuka's trees mix broadleaf-style (rounded canopies) and conifer-style (cones). Monaco also keeps the course's right side free of buildings from Anthony Noghes (T18/19) through to Massenet (T3) — the start/finish straight, Sainte Devote, and the Beau Rivage climb.
...
- **Settings panel**: opened/closed with the "☰" button at the top right (driving pauses while it's open). It bundles drive mode switching (Manual/Assist 25/50/75%/Auto), tilt calibration, a reset button (equivalent to the `R` key), camera/course selection, mute, and vibration on/off
- **Vibration**: the device vibrates (Web Vibration API, `web/src/haptics/vibration.ts`) when you ride a curb, drive on grass, hit the barrier, brake (not at low speed), or exceed the grip limit in a corner (the same moment the cornering-scrub sound starts). Only the strongest applies at a time (barrier > curb > grass > grip limit > brake), and each is a different pulse pattern since the vibration strength itself can't be controlled. It stops while the settings panel is open or the app is in the background. The pulse patterns are tuning constants, not yet confirmed on a real device.
